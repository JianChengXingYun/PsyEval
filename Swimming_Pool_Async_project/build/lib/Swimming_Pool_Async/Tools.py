...
class Tools:
    ...
    def Read_Document_PsyEval(self, num=4):
        self.input_data = []
        existing_contexts = set()
        skipped_count = 0
        processed_count = 0
    ...
    def select_primary_types(self, primary_types,num=4):
        selected_items = []
        
        # 过滤出至少有10条数据的类型
        filtered_types = [typ for typ in primary_types if len(primary_types[typ]) >= num]
        
        if not filtered_types:
            print(f"No types with at least {num} items available.")
            return []
